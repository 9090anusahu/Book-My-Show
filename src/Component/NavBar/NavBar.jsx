...
const NavBar = () => {
    ...
    async  function done(){
        try{
            await axios.post('http://localhost:5000/send-otp',{phoneNumber:formData})
        }
        catch(err){
            console.log(err);
        ...
        <div className='relative overflow-hidden'>
            ...
            <div className='bg-[#55555591] h-full w-full rounded fixed z-10' style={loginTogle ? { display: 'block' } : { display: 'none' }}>
                <div className='w-[420px] h-[550px] p-5 bg-white fixed left-[35%] translate-y-28 rounded flex flex-col items-center'>
                    ...
                    <div className='mt-12'>
                        <button onClick={done} className='w-[300px]'>Continue</button>
                    </div>
                </div>
            </div>
            ...
            <nav className='flex justify-around items-center my-4'>
                ...
                <div className='flex gap-3 items-center'>
                    <li>Jabalpur <RiArrowDropDownLine className='inline text-3xl text-[#555]' /></li>
